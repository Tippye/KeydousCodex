...
2026-09-08 Windows 托盘常驻 Slice：用户要求关闭窗口后程序继续运行。TaskStartSnapshot=main/9acf7bc，Git 干净，仓库已扁平化到工作区根目录。Change Necessity：0.3.2 的 pywebview closing 回调会立即 `begin_close`，与常驻菜单栏语义冲突。范围限定为 Windows 桌面宿主、托盘依赖/打包、窗口生命周期测试、验收脚本与现有说明；不改变 Hook、NJ98 协议、用户 data、宠物或恢复备份。设计：关闭按钮取消销毁并隐藏窗口；系统托盘左键/“打开”恢复并聚焦；托盘“退出”和设置页“退出桥接程序”继续走唯一完整清理路径。托盘初始化失败时拒绝进入可能无法退出的隐藏后台状态。TDD=light，先更新桌面生命周期测试，再实现；ArchitectureReviewRequired=yes，重点复核窗口/托盘/服务三套循环的退出竞态和冻结打包。

2026-09-08 0.4.0 托盘常驻交付检查点：关闭窗口隐藏、托盘默认动作/打开恢复、托盘退出、应用内退出、初始化失败和事件循环异常均收敛到单一 BridgeApp 生命周期。首次冻结验收发现打包图标保留 `icon_256x256.png` 而运行时查找 `app-icon.png`，托盘按设计失败并退出；统一资源名后重复验收通过。最终 101 项 Python 测试中 100 通过、1 项可选真实 Codex 跳过，Node 回归、compileall、diff-check、`Verify-DesktopPackage.py` 的隐藏/存活/重开/明确退出、`Verify-Package.py` 的隔离真实 Hook 生命周期均通过；0.4.0 两个 ZIP 的 SHA-256、CRC、内容与 data/.git/.venv 排除通过。最终用户应用 PID114736，窗口标题 `Keyphore · Keydous`、Responding=True、health=0.4.0；沿用 windows/data，当前 rgb=false，未写设备或 Hook 信任。Complexity Closure=within-budget：desktop.py 保持唯一 Windows UI 生命周期所有者，新增一个托盘线程和一个受控异常分支，无并列后台服务或兼容回退。
